...
class AdvantageEstimator(nn.Module):
    ...
    def discrete_indirect_forward(self, obs, action, alpha):
        state_rep = self.encoder(obs)
        # V(s) = E_{a ~ \pi(s)} [Q(s, a)]
        probs = self.actor(state_rep).probs
        min_q = torch.stack([q(state_rep) for q in self.critics], dim=0).min(0).values
        value = (probs * min_q).sum(1, keepdim=True) - (alpha * probs.entropy())

        # Q(s, a)
        q_preds = (
            torch.stack(
                [q(state_rep).gather(1, action.long()) for q in self.critics], dim=0
            )
            .min(0)
            .values
        ) - (alpha * probs.log_prob(action).sum(-1, keepdim=True))

        # A(s, a) = Q(s, a) - V(s)
        adv = q_preds - value
        return adv

    def continuous_forward(self, obs, action, alpha, n=10):
        ...
        for act in actions:
            q_a_preds = (
                torch.stack([critic(state_rep, act) for critic in self.critics], dim=0)
                .min(0)
                .values
            )
            qs.append(q_a_preds)
        ...
        q_preds = (
            torch.stack([critic(state_rep, action) for critic in self.critics], dim=0)
            .min(0)
            .values
        )
